...
def camera_feed_generator():
    ...
    if not cap.isOpened():
        ...
        # Fall back to simulated feed
        while camera_active:
            img = np.zeros((480, 640, 3), dtype=np.uint8)
            cv2.rectangle(img, (50, 50), (590, 430), (0, 255, 0), 2)
            cv2.putText(img, "Camera Not Available", (200, 240),
...
def start_camera():
    ...
    if not camera_active:
        camera_active = True
        camera_thread = threading.Thread(target=camera_feed_generator)
...
def dashboard():
    """Real-time dashboard"""
    # Start camera automatically when dashboard loads
    if not camera_active:
        start_camera()
...
def start_drunkard_detection():
    """Start drunkard level detection"""
    try:
        # Initialize drunkard detection
        return jsonify({
            'success': True,
...
def get_drunkard_data():
    """Get drunkard detection data"""
    # Simulate alcohol level detection (0.0 to 0.15)
    import random
...
def get_drunkard_status():
    """Get drunkard detection status"""
    return jsonify({
        'detection_active': False,  # You'll need to track this state
        'timestamp': datetime.now().isoformat()
    })


# Driving ability routes
@app.route('/driving_ability')
def driving_ability():
    """Driving ability monitoring page"""
    return render_template('driving_ability.html')


@app.route('/start_monitoring', methods=['POST'])
def start_monitoring():
    """Start driving monitoring"""
    return jsonify({
        'status': 'started',
...
def get_summary():
    ...
    return jsonify({
        'is_running': False,  # You'll need to track this
        'session_start': datetime.now().isoformat(),
        'violations': 0,
        'high_severity': 0,
        'driving_score': 95,
...
def get_profit_data():
    ...
    profit_values = [32000 + random.randint(-2000, 5000) for _ in range(6)]
    expense_categories = ['Fuel', 'Maintenance', 'Driver Salary', 'Insurance', 'Other']
    expense_values = [random.randint(10000, 15000),
                      random.randint(5000, 9000),
                      random.randint(7000, 8500),
                      random.randint(2000, 3000),
                      random.randint(1000, 2000)]

    return jsonify({
        'total_profit': random.randint(40000, 60000),
        'total_revenue': random.randint(70000, 90000),
        'total_expenses': random.randint(25000, 40000),
        'next_month_profit': random.randint(50000, 70000),
        'profit_change': f'+{random.randint(5, 20)}%',
        'revenue_change': f'+{random.randint(3, 15)}%',
        'expense_change': f'+{random.randint(1, 10)}%',
        'prediction_change': f'+{random.randint(10, 25)}%',
